...
interface PaymentButtonProps {
  ...
  productLabel?: string;
  /** 상품 가격 (기본: PAYMENT_CONFIG.price) — 궁합 등 다른 상품에서 오버라이드 */
  price?: number;
}

/** 수동 결제 흐름의 한 단계 (번호 원 + 연결선 + 내용) */
function ManualStep({
  n,
  title,
  last = false,
  ...
  return (
    <div style={{ display: "flex", gap: 12 }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
        ...
        {!last && (
          <div style={{ width: 2, flex: 1, background: "var(--ink-ghost)", margin: "4px 0" }} />
        )}
      </div>
      <div style={{ flex: 1, paddingBottom: last ? 0 : 20, minWidth: 0 }}>
        <p style={{ fontSize: 14, fontWeight: 800, color: "var(--ink)", margin: "3px 0 8px" }}>
          {title}
        </p>
        {children}
      </div>
...
export default function PaymentButton({
  ...
  const [manualCopied, setManualCopied] = useState<"acct" | "msg" | null>(null);
  ...
  const finalPrice = Math.round((price * (100 - discountPct)) / 100);
  ...
  // ── 수동 판매 모드 — 입금 → 알리기 → 코드 입력의 3단계 체크아웃 흐름 ──
  if (!PAYMENT_ENABLED) {
    const acctNumber = BANK_INFO.split(/\s+/).find((t) => /\d{4,}/.test(t)) || BANK_INFO;
    const notifyMsg = `[DESTINO] ${productLabel} 입금 완료했습니다. 입금자명: `;

    const copyText = (text: string, key: "acct" | "msg") => {
    ...
    return (
      <div style={{ width: "100%", maxWidth: "380px", textAlign: "left" }}>
        {/* 1. 입금 */}
        <ManualStep n={1} title={`${finalPrice.toLocaleString()}원 입금하기`}>
          {BANK_INFO ? (
            <div style={cardStyle}>
              <div style={{ fontSize: "15px", fontWeight: 800, color: "var(--ink)", marginBottom: 8 }}>
                {BANK_INFO}
              </div>
              <button onClick={() => copyText(acctNumber, "acct")} style={smallBtnStyle}>
                {manualCopied === "acct" ? "복사됨 ✓" : "계좌번호 복사"}
              </button>
            </div>
          ) : (
            <div style={cardStyle}>
        ...
        {/* 2. 입금 알리기 */}
        <ManualStep n={2} title="입금했다고 알려주기">
          {CONTACT_URL ? (
            <a
              href={CONTACT_URL}
              target="_blank"
            ...
            <div style={cardStyle}>
              ...
              <div style={{ marginTop: 8 }}>
                <button onClick={() => copyText(notifyMsg, "msg")} style={smallBtnStyle}>
                  {manualCopied === "msg" ? "복사됨 ✓" : "보낼 문구 복사"}
                </button>
              </div>
            </div>
          )}
        ...
        {/* 3. 코드 입력 */}
        <ManualStep n={3} title="열람 코드 입력" last>
          <p style={{ fontSize: "12.5px", color: "var(--ink-light)", margin: "0 0 8px" }}>
            확인 즉시 열람 코드를 보내드립니다 (보통 10분 이내) — 받으시면 여기에 입력하세요
          </p>
          ...
          {couponMessage && (
            <p style={{ fontSize: "13px", color: "var(--seal)", marginTop: 8 }}>{couponMessage}</p>
          )}
        </ManualStep>
      </div>
    );
